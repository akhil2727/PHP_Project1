...
<div class="container text-center">
            ...
            <form method="POST" action="checkout.php" class="mt-5">
                <div class="row g-3">
                    ...
                    <div class="col">
                        ...
                        <input type="submit" name="buy" class="btn btn-primary" value="Complete order">
                    </div>
                </div>
            </form>
        <?php } ?>
    <?php
        ...
        if (mysqli_query($conn, $sql)) {
            echo "<h1>New Customer created successfully</h1>";
            $last_id = mysqli_insert_id($conn);
            $sql = "INSERT INTO `bookinventoryorder` (`CUSTOMER_ID`, `BOOK_ID`, `ORDER_DATE`) VALUES ('" . $last_id . "','" . $book . "','" . $date . "')";
            if (mysqli_query($conn, $sql)) {
                echo "<h1>New ORDER created successfully!!</h1><h2>Order ID: " . mysqli_insert_id($conn) . "</h2>";
                $sql = "UPDATE bookinventory SET QUANTITY=QUANTITY-1 WHERE id=".$book;
                if (mysqli_query($conn, $sql)) {
